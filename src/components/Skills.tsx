...
export default function Skills() {
  ...
  return (
    <section id="skills" className="py-20 bg-slate-800" ref={ref}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {category.skills.map((skill, skillIndex) => (
                    <motion.div
                      ...
                      className="flex flex-col items-center gap-2 p-3 bg-white/5 rounded-lg border border-white/5 hover:border-white/20 transition-all duration-300 group"
                    >
                      <skill.icon
                        className="w-8 h-8 transition-all duration-300"
                        style={{ color: skill.color }}
                      />
                      <span className="text-xs sm:text-sm text-gray-300 text-center font-medium group-hover:text-white transition-colors">
                        {skill.name}
                      </span>
                    </motion.div>
                  ))}
            ...
            <div className="bg-gradient-to-r from-blue-500/10 to-cyan-500/10 backdrop-blur-sm border border-blue-500/20 rounded-2xl p-6">
              ...
              <div className="flex flex-wrap gap-3">
                {['Problem Solving', 'Collaboration', 'Adaptability', 'Team Work', 'Communication', 'Agile Methodologies'].map((skill) => (
                  <span
                    key={skill}
                    className="px-4 py-2 bg-white/10 border border-white/20 rounded-full text-gray-300 text-sm hover:bg-white/20 transition-colors"
                  >
                    {skill}
                  </span>
                ))}
